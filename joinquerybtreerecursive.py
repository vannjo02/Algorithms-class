...
class BTreeNode:
    ...
    def insert(self,bTree,item):
        ...
        if not self.isFull():
            x = 0
            y = True 
            while self.items[x] < item and y:
                x += 1 
                if self.items[x] == None:
                    y = False
                
            self.items.insert(x,item)
            self.items = self.items[0,:]
            self.items.setNumberOfKeys += 1
            #slice off the none
        else:
            self.items.append(item)
            self.items.sort()
            #Case 1 two new nodes
            #Case 2 one new node
            if self.child[0] == None and self.parent == None:
                newPar = BTreeNode(bTree.degrees)
                newSib = BTreeNode(bTree.degrees)
                temp = self.items[2]
                newPar.items.append(self.items[2])
                newPar.items.sort()
                newSib.items.append(self.items[3])
                newSib.items.append(self.items[4])
                self.items = self.items[0:2]
                self.items.setNumberOfKeys = 2
                newPar.items.setNumberOfKeys = 1
                newSib.items.setNumberOfKeys = 2
                newPar.child.setChild(self,newPar.items.index(temp))
                newPar.child.setChild(self,newPar.items.index(temp)+1)
                newSib.parent = newPar
                self.parent = newPar
            else:
                newSib = BTreeNode(bTree.degrees)
                newSib.items.append(self.items[3])
                newSib.items.append(self.items[4])
                self.items = self.items[0:2]
                newSib.setNumberOfKeys = 2
                self.setNumberOfKeys = 3
    ...
    def delete(self,bTree,item):
        ...
        '''
        if self.rootNode.search(bTree,item):
            self.items.remove(item)
            self.setNumberOfKeys -= 1
            if self.getNumberOfKeys < 2:
                self.redistributeOrCoalesce(bTree,self.getNumberOfKeys)
        else:
            return None
    ...
    def redistributeOrCoalesce(self,bTree,childIndex):
        ...
        if bTree(childIndex).numberofkeys < 2:
            if sibling.isFull != True:
                temp = getChild(childIndex)
                delete(bTree, childIndex)
                sibling.insert(bTree, temp)
    ...
    def search(self, bTree, item):
        ...
        while ((not found) and (count < self.numberOfKeys)):
            if self.rootNode.items[count] != item:
                if self.rootNode.items[count] > searchItem:
                    nodeIndex = count
        ...
        if count == self.numberOfKeys:
            nodeIndex = self.numberOfKeys
        search_dict = {'found': found, 'nodeIndex': nodeIndex}
        return search_dict
        '''
        
        
        if self.rootNode.items.index(item) != None:
            return self.index
        if self == self.rootNode and self.rootNode.items.index(item) == None:
            return None
        if self.items.index(item) != None:
    
            return self.index
        else:
            self.Parent(self).search(bTree,item)
...
class BTree:
    ...
    def delete(self, anItem):
        ...
        ''' 
        if self.search(anItem) == None:
            return None
        else:
            self.delete(bTree, anItem)
    ...
    def insert(self, anItem):
        ...
        '''
        #Case 1, new tree
        #Case 2, anItem is in tree
        #Case 3, anItem is not in tree
        if self.rootNode.getNumberOfKeys == 0:
            self.rootNode.insert(self,anItem)
        else:
            if not self.retrieve(anItem) == None:
                return None
            else:
                self.rootNode.search(self,anItem).insert(self,anItem)
    ...
    def retrieve(self, anItem):
        ...
        if __searchTree(anItem):
            return deepcopy(anItem)
        else:
            return None
