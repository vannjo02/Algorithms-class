...
class Heap:
  ...
  def bestChildOf(self, index, lastIndex):
    ...
    bestChild = None
    if self.largestOnTop:
      bestChild = self.data[0]
      for y in (1, lastIndex):
        if self.data[y] > bestChild:
          bestChild = self.data[y]
    else:
      bestChild= self.data[0]
      for y in (1, lastIndex):
        if self.data[y] < bestChild:
  ...
  def removeTop(self):
    ...
    if self.data == None:
      ...
      x = bestChildOf(self, 1, self.capacity-1)
      siftUpFrom(x)
  ...
  def siftDownFrom( self, fromIndex ):
    ...
    '''
    if self.largestOnTop:
      while self.data[fromIndex] < self.data[0]:
      
        Childindex = (fromIndex * self.maxChildren)
        if self.data[fromIndex] > self.data[fromIndex]:
          temp=self.data[Childindex]
          self.data[Childindex] = self.data[fromIndex]
          self.data[fromIndex] = temp
          __siftDownFrom(self, Childindex)
    else:
      while self.data[fromIndex] > self.data[0]:
        Childindex = (fromIndex)//self.maxChildren
        if self.data[fromIndex] < self.data[fromIndex]:
  ...
  def __siftUpFrom(self, child):
    ...
    if self.largestOnTop:
      while self.data[child] < self.data[0]:
      
        Parentindex = (child)//self.maxChildren
        if self.data[Parent] > self.data[child]:
          temp=self.data[Parentindex]
          self.data[Parentindex] = self.data[child]
          self.data[child] = temp
          __siftUpFrom(self, Parentindex)
    else:
      while self.data[child] > self.data[0]:
        Parentindex = (child)//self.maxChildren
        if self.data[Parent] < self.data[child]:
